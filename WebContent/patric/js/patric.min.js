...
function search_all_header() {
	if (Ext.String.trim(Ext.getDom("global_search_keyword").value) == "" || Ext.getDom("global_search_keyword").value == "*") {
		...
		Ext.Ajax.request({
			url : "/GenomicFeatureWindow?action=b&cacheability=PAGE",
			method : "POST",
			params : {
			...
			success : function (a) {
				document.location.href = "GlobalSearch?cType=taxon&cId=&dm=&pk=" + a.responseText+"&keyword="+EncodeKeyword(Ext.getDom("global_search_keyword").value);
			}
		})
	}
}
